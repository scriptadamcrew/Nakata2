...
wget -q -O /usr/local/bin/edu-ssh https://raw.githubusercontent.com/scriptadamcrew/Nakata2/main/proxy-template.py
chmod +x /usr/local/bin/edu-ssh

# Installing Service
cat > /etc/systemd/system/edussh-nontls.service << END
[Unit]
Description=Ssh Websocket By Nakata
...
wget -q -O /usr/local/bin/edu-ovpn https://raw.githubusercontent.com/scriptadamcrew/Nakata2/main/edu-op.py
...
[Unit]
Description=Ovpn Websocket By Nakata
Documentation=Nakata
After=network.target nss-lookup.target
